<x-layout>
    {{-- SEO Meta Tags --}}
    @section('meta_title', $property->title . ' | ' . ($settings['site_name'] ?? 'MadeInTravel'))
    @section('meta_description', 'For ' . $property->listing_type . ': ' . $property->title . ' in ' . $property->city . '. Price: Rp ' . number_format($property->price, 0, ',', '.'))
    @section('requires_leaflet', true) {{-- Tells the layout to load Leaflet CSS/JS --}}
    ...
    @push('head')
        ...
        <meta property="og:site_name" content="{{ $settings['site_name'] ?? 'MadeInTravel' }}" />
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        @php
            // Filter out the 360 tour images so they don't show in the standard slider
            $galleryImages = $property->media->where('file_type', '!=', 'VIRTUAL_TOUR_360')->values();
            $imageUrls = $galleryImages->map(fn($m) => asset('storage/' . $m->file_path));
        @endphp
        ...
        class="mb-8">
            ...
            {{-- Sliding Carousel --}}
            <div class="relative h-[350px] md:h-[500px] w-full rounded-2xl overflow-hidden group shadow-sm border border-gray-200 bg-gray-100">
                
                {{-- Image Track --}}
                <div class="flex w-full h-full transition-transform duration-700 ease-out"
                     :style="`transform: translateX(-${activeImage * 100}%)`">
                    <template x-for="(img, index) in images" :key="index">
                        <div class="w-full h-full flex-shrink-0 relative cursor-zoom-in"
                ...
                {{-- Arrows & Indicators (Only show if multiple images) --}}
                <template x-if="images.length > 1">
                    <div>
                        {{-- Left Arrow --}}
                        <button @click.prevent="prev()" class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-gray-800 p-3 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-sm z-10 focus:outline-none transform -translate-x-4 group-hover:translate-x-0">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                        </button>
                        
                        {{-- Right Arrow --}}
                        <button @click.prevent="next()" class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-gray-800 p-3 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-sm z-10 focus:outline-none transform translate-x-4 group-hover:translate-x-0">
                        ...
                        <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 z-10">
                            <template x-for="(_, index) in images" :key="index">
                                <button @click="activeImage = index"
                                        class="h-2.5 rounded-full transition-all duration-300 shadow-sm focus:outline-none"
            ...
            <div x-show="lightboxOpen" 
                ...
                <template x-if="images.length > 1">
                    <div>
                        <button @click="prev()" class="absolute left-4 text-white hover:text-gray-300 z-50 p-3 bg-white/10 rounded-full hover:bg-white/20 transition">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                        </button>

                        <button @click="next()" class="absolute right-4 text-white hover:text-gray-300 z-50 p-3 bg-white/10 rounded-full hover:bg-white/20 transition">
        ...
        @else
            ...
            <div class="h-[350px] md:h-[500px] w-full rounded-2xl bg-gray-200 border border-gray-300 flex flex-col items-center justify-center text-gray-400 mb-8 shadow-sm">
                <svg class="w-16 h-16 mb-2 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                <span class="font-medium">{{ __('No images available') }}</span>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
            ...
            <div class="md:col-span-2 space-y-8">
                ...
                @if($property->youtube_url)
                    <div class="mt-8">
                        <h2 class="text-xl font-bold text-gray-900 mb-4">{{ __('Video Tour') }}</h2>
                ...
                <div class="bg-white p-6 rounded-xl shadow-lg border mt-8">
                    ...
                    <div class="space-y-4" x-data="kprCalculator()">
                        <div>
                            ...
                            <select x-model="selectedBankId" @change="updateBank()" class="w-full border border-gray-300 rounded-lg py-2 px-3 mt-1 outline-none focus:ring-2 focus:ring-sky-500">
                                @foreach($banks as $bank)
                                    <option value="{{ $bank->id }}">{{ $bank->name }} (Floating: {{ $bank->floating_rate }}%)</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select x-model="tenor" class="w-full border border-gray-300 rounded-lg mt-1 py-2 px-3 outline-none">
                                    <template x-for="y in (currentBank.max_tenor || 20)"><option :value="y" x-text="y + ' Years'"></option></template>
                                </select>
                            </div>
                        </div>
                        <div class="bg-sky-50 p-4 rounded-lg text-center mt-4 border border-sky-100">
                            ...
                            <p class="text-[10px] text-gray-500 mt-2">*Using <span x-text="effectiveRate"></span>% Effective Rate (Avg of Fixed & Floating)</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="md:col-span-1">
                <div class="bg-white p-6 rounded-xl shadow-lg border sticky top-24">
                    <div class="flex items-center space-x-4 mb-6">
                        @if($property->user->avatar_url)
                            <img src="{{ asset('storage/' . $property->user->avatar_url) }}" 
                                class="w-24 h-24 rounded-full object-cover border border-sky-200" alt="Avatar">
                        @else
                        ...
                        <div>
                            ...
                            <a href="{{ route('property.agent.show', $property->user->id) }}" class="hover:underline">
                    ...
                    <div x-data="{ 
                            viewingOpen: false,
                            date: '', time: '', phone: '{{ $phone }}', title: '{{ $property->title }}', url: '{{ url()->current() }}',
                            get whatsappLink() {
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-rose-600 text-sm mb-4">
                                        {{ __('This property is available for rent and can be booked as a venue through our Event Organizer service.') }}
                                    </p>
                        ...
                        <div x-show="viewingOpen" style="display: none;" class="fixed inset-0 z-[9999] flex items-center justify-center px-4"
                            ...
                            <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-md p-6 overflow-hidden transform transition-all" x-transition.scale.95>
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <input type="date" x-model="date" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-500 outline-none" min="{{ date('Y-m-d') }}">
                                    </div>
                                    <div>
                                ...
                                <div class="mt-8 pt-4 border-t border-gray-100 flex gap-3">
                                    <button @click="viewingOpen = false" class="w-1/3 px-4 py-3 text-gray-700 font-bold hover:bg-gray-100 rounded-lg transition">{{ __('Cancel') }}</button>
                                    <a :href="whatsappLink" target="_blank" @click="viewingOpen = false" class="w-2/3 bg-sky-500 hover:bg-sky-600 text-white font-bold py-3 rounded-lg transition flex items-center justify-center gap-2 shadow-lg hover:shadow-sky-500/30">
                                        {{ __('Confirm & Chat') }}
                                    </a>
                                </div>
                            </div>
                    ...
                    target="_blank" class="block w-full border border-sky-600 text-sky-600 hover:bg-sky-50 font-bold py-3 rounded-lg transition text-center mt-3">
                    ...
                    <div class="mt-6 border-t pt-4">
                        ...
                        @endif
                        <form action="{{ route('property.inquiry.store', $property->id) }}" method="POST" class="space-y-4">
                            @csrf 
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('Name') }}</label>
                                <input type="text" name="name" value="{{ old('name') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('Email') }}</label>
                                <input type="email" name="email" value="{{ old('email') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('Phone') }}</label>
                                <input type="text" name="phone" value="{{ old('phone') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('Message') }}</label>
                                <textarea name="message" rows="4" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500">{{ old('message', __('I am interested in this property.')) }}</textarea>
                            </div>
                ...
                function kprCalculator() {
                    return {
                        ...
                        dp_percent: 20, tenor: 15,
                        banks: {{ Js::from($banks) }},
                        selectedBankId: {{ $banks->first()->id ?? 'null' }},
                        currentBank: {}, effectiveRate: 0,
                        init() { this.updateBank(); },
                        updateBank() {
                        ...
                        <div class="relative h-64 overflow-hidden">
                            <a href="{{ route('property.show', ['id' => $related->id, 'slug' => $related->slug]) }}">
                                @if($related->media->first())
                                    <img src="{{ asset('storage/' . $related->media->first()->file_path) }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                                @else
                                    <div class="w-full h-full bg-gray-200 flex items-center justify-center text-gray-400">{{ __('No Image') }}</div>
                                @endif
                            </a>
                            <div class="absolute top-4 left-4">
                                <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm {{ $related->listing_type == 'SALE' ? 'bg-sky-600 text-white' : 'bg-emerald-500 text-white' }}">
                                    {{ __($related->listing_type) }}
                                </span>
                            </div>
                            <div class="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-lg shadow-sm border border-white/50">
                                <span class="text-sky-700 font-extrabold text-lg">Rp {{ number_format($related->price, 0, ',', '.') }}</span>
                            </div>
                        </div>
                        <div class="p-5">
                            <div class="flex items-center text-xs text-gray-500 mb-2">📍 {{ $related->city }}, {{ $related->district }}</div>
                            <h3 class="font-bold text-gray-900 text-lg leading-tight mb-4 group-hover:text-sky-600 transition-colors">
                                <a href="{{ route('property.show', ['id' => $related->id, 'slug' => $related->slug]) }}">{{ Str::limit($related->title, 45) }}</a>
                            </h3>
                            <div class="flex items-center justify-between border-t border-gray-100 pt-4 text-sm text-gray-600">
                                <div class="flex items-center gap-1"><span class="font-semibold">{{ $related->bedrooms }}</span> <span class="text-xs">{{ __('Bed') }}</span></div>
                                <div class="flex items-center gap-1"><span class="font-semibold">{{ $related->bathrooms }}</span> <span class="text-xs">{{ __('Bath') }}</span></div>
                                <div class="flex items-center gap-1"><span class="font-semibold">{{ $related->building_area }}</span> <span class="text-xs">m²</span></div>
                            </div>
                        </div>
                    </div>
                @endforeach
