...
<head>
    ...
    <title>  {{ $settings['site_name'] ?? 'MadeInTravel' }}</title>
...
<body class="bg-gray-50 overflow-hidden">
    ...
    <nav class="absolute top-0 left-0 w-full bg-white/90 backdrop-blur-md border-b border-gray-200 h-16 z-[1000] shadow-sm flex-shrink-0">
        <div class="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
            <a href="{{ route('property.home') }}" class="text-xl font-extrabold text-sky-600 tracking-tight">                    {{ $settings['site_name'] ?? 'MadeInTravel' }}</a>
            
            <div class="flex gap-3">
                <a href="{{ route('property.home') }}" class="flex items-center gap-2 bg-white border border-gray-300 hover:border-sky-500 hover:text-sky-600 px-4 py-1.5 rounded-full text-sm font-medium transition shadow-sm">
