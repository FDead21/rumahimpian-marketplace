<x-layout>
    ...
    <div class="max-w-3xl mx-auto px-4 py-16 print-wrapper">
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden print-card">
            ...
            <div class="p-6 md:p-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    ...
                    <div>
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{{ __('Rental Details') }}</h4>
                        <p class="text-gray-700"><span class="font-bold">{{ __('Vehicle') }}:</span> {{ $booking->rentalVehicle->name }}</p>
                        ...
                        <p class="text-gray-700"><span class="font-bold">{{ __('Duration') }}:</span> {{ $days }} {{ __('Days') }}</p>
                        <p class="text-gray-700"><span class="font-bold">{{ __('Start Date') }}:</span> {{ $start->translatedFormat('d M Y') }}</p>
                        <p class="text-gray-700"><span class="font-bold">{{ __('End Date') }}:</span> {{ $end->translatedFormat('d M Y') }}</p>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr>
                                <td class="py-4">
                                    <p class="font-bold text-gray-900">{{ $booking->rentalVehicle->name }}</p>
                                    <p class="text-xs text-gray-500">
                                        Rp {{ number_format($booking->rentalVehicle->price_per_day, 0, ',', '.') }} × {{ $days }} {{ __('days') }}
                                    </p>
                                </td>
            ...
            @php
                // Build WA Link logic (adjust the phone number if needed)
                $waPhone = '6281234567890'; // Replace with your standard admin/vendor number or fetch dynamically
                $intro = __("Halo! Saya telah melakukan booking rental kendaraan:");
                $waMessage = "$intro\n\n" .
                    "*" . __('Kode') . ":* {$booking->booking_code}\n" .
                    "*" . __('Kendaraan') . ":* {$booking->rentalVehicle->name}\n" .
                    "*" . __('Durasi') . ":* {$days} Hari ({$start->format('d/m/Y')} - {$end->format('d/m/Y')})\n" .
                    "*" . __('Total') . ":* Rp " . number_format($booking->total_price, 0, ',', '.') . "\n\n" .
                    __("Mohon konfirmasi pesanan saya. Terima kasih!");
        ...
        <div class="text-center mt-8 no-print">
            <a href="{{ route('rental.vehicles.index') }}" class="text-emerald-600 font-bold hover:underline">
                &larr; {{ __('Back to Rentals') }}
            </a>
        </div>
    </div>
...
<style>
    @media print {
        ...
        .print-card { page-break-inside: avoid !important; break-inside: avoid !important; box-shadow: none !important; border: 1px solid #e5e7eb !important; margin: 0 !important; }
