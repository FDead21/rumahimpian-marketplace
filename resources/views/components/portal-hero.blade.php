@php
    ...
    $initialTab = $isEo ? "'events'" : "'property'";
@endphp

{{-- ========================================================= --}}
{{-- TRUE UNIFIED GLOBAL HERO SECTION (Alpine Powered)         --}}
{{-- ========================================================= --}}
<div class="relative bg-slate-900 min-h-[70vh] md:min-h-[85vh] flex flex-col justify-center pt-20" x-data="{ activeTab: {{ $initialTab }} }">  {{-- Dynamic Background Image --}}
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center md:text-left mb-8">
            <h1 class="text-4xl md:text-6xl font-extrabold text-white mb-4 drop-shadow-lg tracking-tight"
                x-text="activeTab === 'property' ? '{{ __('Find Your Perfect Home in Indonesia') }}' : '{{ __('Unforgettable Events, Perfectly Organized') }}'">
            </h1>
            <p class="text-gray-200 text-lg md:text-xl drop-shadow-md"
               x-text="activeTab === 'property' ? '{{ __('Search thousands of verified listings.') }}' : '{{ __('Premium venues, top catering, and all-in-one packages.') }}'">
            </p>
        </div>

        {{-- ONE UNIFIED SMART FORM --}}
        <div class="w-full max-w-5xl">
            {{-- Notice the dynamic :action based on the tab! --}}
            <form method="GET" class="flex flex-col md:flex-row w-full bg-white rounded-2xl shadow-2xl p-1 gap-1"
                  :action="activeTab === 'property' ? '{{ route('property.home') }}' : '{{ route('eventOrganizer.packages.index') }}'">
                ...
                <div class="flex-1 relative flex items-center bg-transparent border-b md:border-b-0" :class="activeTab === 'property' ? 'md:border-r border-gray-200' : ''">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           :placeholder="activeTab === 'property' ? '{{ __('City, district, or property name') }}' : '{{ __('Search packages or vendors') }}'"
