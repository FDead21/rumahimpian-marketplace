...
<div class="bg-white">
    ...
    <div class="max-w-6xl mx-auto px-4 mb-8"
                    ...
                    <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 z-10">
                        <template x-for="(_, index) in images" :key="index">
                            <button @click="activeImage = index"
                                    ...
                                    :class="activeImage === index ? 'w-8 bg-rose-600' : 'w-2.5 bg-white/70 hover:bg-white'">
                            </button>
                        </template>
                    </div>
                </div>
        ...
        <div x-show="lightboxOpen"
            ...
            <button @click="lightboxOpen = false" class="absolute top-5 right-5 text-white hover:text-gray-300 z-50 focus:outline-none">
            ...
            <template x-if="images.length > 1">
                <div>
                    <button @click="prev()" class="absolute left-4 z-50 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition focus:outline-none">
                    ...
                    <button @click="next()" class="absolute right-4 z-50 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition focus:outline-none">
    ...
    <div class="max-w-6xl mx-auto px-4 pb-16">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div>
                    <h2 class="text-xl font-bold text-gray-900 mb-3">{{ __('About This Package') }}</h2>
                ...
                <div class="pt-6 border-t border-gray-100">
                    <h2 class="text-xl font-bold text-gray-900 mb-6">{{ __('Vendors in this Bundle') }}</h2>
                            ...
                            @foreach($package->inclusions as $index => $inclusion)
                                <div x-show="activeIdx === {{ $index }}" 
                                    ...
                                    <div class="relative mt-auto p-6 md:p-8 text-white z-10">
                                        <span class="inline-block bg-rose-600 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider mb-3">{{ __('Feature') }} {{ $index + 1 }}</span>
                        ...
                        @foreach($package->inclusions as $index => $inclusion)
                            <div x-show="activeIdx === {{ $index }}" class="w-full max-w-6xl flex flex-col md:flex-row gap-8 items-center"
                                ...
                                <div class="w-full md:w-2/5 text-white bg-gray-900/50 p-8 rounded-3xl border border-gray-800">
                                    <span class="text-rose-500 font-bold text-sm tracking-widest uppercase mb-2 block">{{ __('Feature') }} {{ $index + 1 }}</span>
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-6 bg-white border border-gray-200 rounded-2xl shadow-lg p-6 space-y-4">
                    <div class="bg-rose-50 border border-rose-100 rounded-xl px-5 py-4">
                        <p class="text-sm text-rose-500 font-semibold mb-1">{{ __('Starting from') }}</p>
                        @if($package->original_price && $package->original_price > $package->price)
                            <div class="flex items-center gap-2 mb-1">
                                <span class="bg-rose-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                                    {{ __('Hemat') }} Rp {{ number_format($package->discount_amount, 0, ',', '.') }}
                                </span>
                            </div>
                    ...
                    <div class="flex items-center gap-3 px-2 py-2">
                        @php
                            $avatarSrc = $package->user->avatar_url 
                                ? (str_starts_with($package->user->avatar_url, 'http') ? $package->user->avatar_url : asset('storage/' . $package->user->avatar_url))
                                : null;
                        @endphp
                        @if($avatarSrc)
    ...
    <div class="bg-gray-50 border-t border-gray-100 py-14">
        <div class="max-w-6xl mx-auto px-4">
            <h2 class="text-2xl font-bold text-gray-900 mb-8">{{ __('Other Packages You Might Like') }}</h2>
